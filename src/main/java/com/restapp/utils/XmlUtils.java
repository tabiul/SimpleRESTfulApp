...
public class XmlUtils {
    public static final DateFormat sdf = new SimpleDateFormat("dd/MM/yyyy HH:mm");
    ...
    public static Map<String, List<Sighting>> readSightingFile(
        ...
        for (int i = 0; i < nList.getLength(); i++) {
            ...
            if (nNode.getNodeType() == Node.ELEMENT_NODE) {
                ...
                String location = eElement.getElementsByTagName("location").item(0)
                        .getTextContent();
                Date date = sdf.parse((eElement.getElementsByTagName("date")
                        .item(0).getTextContent()));
                if (sightingsMap.containsKey(birdName)) {
                    sightingsMap.get(birdName).add(new Sighting(location, date));
                } else {
    ...
    public static void writeData(final List<Bird> birds,
        ...
        try {
            ...
            Element rootElementSighting = docSighting.createElement("sightings");
            ...
            for (Bird bird : birds) {
                ...
                colorElement.appendChild(docBird.createTextNode(bird.getColor()));
                ...
                for (Sighting sighting : bird.getSightings()) {
                    Element sightingElement = docSighting.createElement("sighting");
                    rootElementSighting.appendChild(sightingElement);
                    sightingElement.setAttribute("birdname", bird.getName());
                    Element locationElement = docSighting.createElement("location");
                    locationElement.appendChild(docSighting.createTextNode(sighting
                            .getLocation()));
                    sightingElement.appendChild(locationElement);

                    Element dateElement = docSighting.createElement("date");
                    dateElement.appendChild(docSighting.createTextNode(sdf
